...
//update	
    ...
    public function mdplayer_del()
    {
        ...
        if (count($cid)) {
            ...
            /*$db->setQuery('SELECT CONCAT(first_name," ", last_name) as name,id FROM #__bl_players WHERE id IN ('.$cids.') AND created_by = '.$user->id);
            $playersA = $db->loadObjectList();
            
            for($intA=0;$intA<count($playersA);$intA++){
                $errorStr = date("d-m-Y H:i:s")." - player ".$playersA[$intA]->name." (id:".$playersA[$intA]->id.") remove by user : ".$user->id." from moderator \n";
                error_log($errorStr, 3, JPATH_ROOT."/players_log.log");
            }*/
