...
function joomsportBuildRoute(&$query)
{
    ...
    if (isset($query['usrid'])) {
        $segments[] = $query['usrid'];
        unset($query['usrid']);
    };
...
function joomsportParseRoute($segments)
{
    ...
    switch ($segments[0]) {
        ...
        case 'bet_matches':
            ...
            break;
        case 'tournamentlist':
            $vars['view'] = 'tournamentlist';
            $vars['task'] = 'tournamentlist';
            break;
        case 'userleague':
                $vars['task'] = 'userleague';
                $id = explode(':', $segments[1]);
                $vars['id'] = (int) $id[0];
                if(isset($segments[2])){
                    $id = explode(':', $segments[2]);
...
function toAscii($clean, $replace=array(), $delimiter='-') {
	if( !empty($replace) ) {
		$str = str_replace((array)$replace, ' ', $clean);
	}
