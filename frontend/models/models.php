...
class JSPRO_Models
{
    ...
    {
        $query = "SELECT s.s_id as id, CONCAT(t.name,' ',s.s_name) as name,t.t_single,s.s_enbl_extra,t.tournament_type,s.season_options FROM #__bl_tournament as t, #__bl_seasons as s WHERE s.s_id = ".intval($sid).' AND s.t_id = t.id ORDER BY t.name, s.s_name';
        $this->db->setQuery($query);
...
///////////////////////
        ...
        if ($showSelect || count($seass)) {
            ...
            if(!count($seasplayed)){
                $jqre = $jqre2 = JText::_('BLFA_NOT_SEASONNULL').'<input type="hidden" name="moderseason" id="moderseason" value="0" />';
            }
