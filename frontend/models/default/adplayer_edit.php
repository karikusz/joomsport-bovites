    ...
    public function delAdmPlayer()
    {
        ...
        if ($this->acl == 1) {
            ...
            if (count($cid)) {
                ...
                /*$user = &JFactory::getUser();
                $db->setQuery('SELECT CONCAT(first_name," ", last_name) as name,id FROM #__bl_players WHERE id IN ('.$cids.')');
                $playersA = $db->loadObjectList();

                for($intA=0;$intA<count($playersA);$intA++){
                    $errorStr = date("d-m-Y H:i:s")." - player ".$playersA[$intA]->name." (id:".$playersA[$intA]->id.") remove by user : ".$user->id." from FE admin \n";
