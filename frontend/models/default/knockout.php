...
//echo $marg."  ---   ".$marg1;
    ...
    public static function getKnockComplpex($row, $tourn, $match, $s_id, $get_kn_cfg, $type){
        ...
        $db->setQuery("SELECT kn_winner FROM #__bl_matchday WHERE id={$row->id}");
        $kn_winner = intval($db->loadResult());
                ...
                for($intB=0; $intB < $stages; $intB ++){
                    if($intA == 0 || ($intA % (pow(2,$intB)))==0){
                        ...
                        if($intB < $stages - 1){
                            ...
                            echo '<i class="jsproceednext jsknockfinal fa fa-arrow-circle-right'.$morefaclass.'" aria-hidden="true"></i>';
                            echo '<input type="hidden" id="jsknock_winnerid" name="jsknock_winnerid" value="'.$kn_winner.'" />';
                        }
                        ...
                        if($intB == 0){
                            echo JHTML::_('select.genericlist',   $teamis, "set_home_team_".$intA."_".$intB, 'class="js_selpartic js_selpartichome" data-chosen="kn" size="1" id="js_selpartic_'.$intA.'_'.$intB.'"', 'id', 't_name', $kvalues["home"], 'js_selpartic_'.$intA.'_'.$intB);
                            ...
                            if($kvalues["home"] > 0){
                                ...
                                if($kn_winner && ($intB >= $stages - 1) && ($kn_winner == $kvalues["home"])){
                        ...
                        if($kvalues["match_id"] && count($kvalues["match_id"])){
                            ...
                            foreach ($kvalues["match_id"] as $kmid) {
                                //
                                $db->setQuery("SELECT * FROM #__bl_match WHERE id=".intval($kmid));
                                $match = $db->loadObject();
                                if(isset($match->id) && $match->m_played == 1){
                                    $kvalues["score1"][$intZ] = $match->score1;
                                    $kvalues["score2"][$intZ] = $match->score2;
                                }
                                //
                        ...
                        if($intB == 0){
                            ...
                            echo JHTML::_('select.genericlist',   $teamis, "set_away_team_".$intA."_".$intB, 'class="js_selpartic js_selparticaway" data-chosen="kn" size="1" id="js_selpartic2_'.$intA.'_'.$intB.'"', 'id', 't_name', $kvalues["away"], 'js_selpartic2_'.$intA.'_'.$intB);
                            ...
                            if($kvalues["away"] > 0){
                                ...
                                if($kn_winner && ($intB >= $stages - 1) && ($kn_winner == $kvalues["away"])){
                                    echo '<div class="jsknockwinnerDiv"></div>';
                                }
                ...
                for($intB=0; $intB < $stages; $intB ++){
                    if($intA == 0 || ($intA % (pow(2,$intB)))==0){
                        ...
                        if($intB < $stages - 1){
                            ...
                            echo '<i class="jsproceednext jsknockfinal fa fa-arrow-circle-right'.$morefaclass.'" aria-hidden="true"></i>';
                            echo '<input type="hidden" id="jsknock_winnerid" name="jsknock_winnerid" value="" />';
                        }
                        //echo '<i class="fa fa-cog jsmatchconf" aria-hidden="true"></i>';
                        echo '<div class="player knocktop ml9">'
                                . '<div class="kntmprow">'
