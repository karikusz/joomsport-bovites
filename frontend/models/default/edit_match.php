...
class edit_matchJSModel extends JSPRO_Models
{
    ...
    public function getPlEvent($id)
    {
        ...
        $this->_lists['events'] = JHTML::_('select.genericlist',   $is_event, 'event_id', 'class="selectpicker" size="1" onchange="jsGetSubEventMatch();" style="width:130px;"', 'id', 'e_name', 0);
        ...
        $query = "SELECT me.*,ev.e_name,CONCAT(p.first_name,' ',p.last_name) as p_name,(subev.e_id) as subevID, GROUP_CONCAT(CONCAT(subev.player_id,'*',subev.t_id)) as subevPl, GROUP_CONCAT(CONCAT(pSub.first_name,' ',pSub.last_name)) as plFM"
            ...
            . ' JOIN #__bl_events as ev ON me.e_id = ev.id AND me.match_id = '.$id
    ...
    public function getMPlayers(&$row)
    {
        if ($this->t_single) {
            ...
            $jqreHd1 = '<select name="playerz_id" style="width:150px;" id="playerz_id"  size="1">';
            $jqreHd1 .= '<option value="">'.JText::_('BLFA_SELPLAYER').'</option>';
            $jqre = '';
            ...
            //$jqre .= '</select>';
            $this->_lists['players'] = $jqreHd1 . $jqre . '</select>';

            $jqreHd2 = '<select name="playerzSub_id[]" id="playerzSub_id" style="width:120px;" multiple class="chzn-done" size="1" data-placeholder="Select player">';

            $this->_lists['playersSub'] = $jqreHd2.$jqre.'</select>';
    ...
    public function saveAdmmatch()
    {
        ...
        $row_prev = new JTableMatch($this->db);
        $row_prev->load(intval($_POST["id"]));
        $score1_prev = $row_prev->score1;
        $score2_prev = $row_prev->score2;
        ...
        $this->db->setQuery("SELECT id, player_event FROM #__bl_box_fields WHERE complex='0' AND ftype = '2' AND player_event > 0");
        $event_box_fields_ = $this->db->loadObjectList();
        $event_box_fields = array();
        foreach($event_box_fields_ as $item) {
            $event_box_fields[$item->player_event] = $item->id;
        }
        $events_boxFields = array_keys($event_box_fields);
        $event_box_data = array();
        ...
        if (isset($_POST['new_eventid']) && count($_POST['new_eventid'])) {
            for ($i = 0; $i < count($_POST['new_eventid']); ++$i) {
                if (!isset($_POST['em_id'][$i]) || !intval($_POST['em_id'][$i])) {
                    ...
                    $me_arr[] = $curev = $this->db->insertid();
                    if (in_array($new_event, $events_boxFields)) {
                        if (isset($event_box_data[intval($plis[0])][$new_event])) {
                            $event_box_data[intval($plis[0])][$new_event] += intval($_POST['e_countval'][$i]);
                        } else {
                            $event_box_data[intval($plis[0])][$new_event] = intval($_POST['e_countval'][$i]);
                        }
                    }
                    ...
                    //subevents
                    if(isset($_POST['sub_eventid_vals'][$i]) && $_POST['sub_eventid_vals'][$i]){
                        $plisSubArr = explode(',', $_POST['sub_eventid_vals'][$i]);
                        $eorderingSub = 0;
                        if(count($plisSubArr)){
                    ...
                    if (count($event_bl)) {
                        ...
                        $query = 'SELECT * FROM #__bl_match_events WHERE additional_to='.intval($_POST['em_id'][$i]);
                        $this->db->setQuery($query);
                        $adit = $this->db->loadObjectList();
                        for($intP=0;$intP<count($adit);$intP++){
                            $me_arr[] = $adit[$intP]->id;
                        }

                        if (in_array($event_bl[0]->e_id, $events_boxFields)) {
                            if (isset($event_box_data[$event_bl[0]->player_id][$event_bl[0]->e_id])) {
                                $event_box_data[$event_bl[0]->player_id][$event_bl[0]->e_id] += intval($_POST['e_countval'][$i]);
                            } else {
                                $event_box_data[$event_bl[0]->player_id][$event_bl[0]->e_id] = intval($_POST['e_countval'][$i]);
                            }
                        }
        ...
        for($intA=0;$intA<count($h_players);$intA++){
            ...
            if (isset($event_box_data[$h_players[$intA]->id]) && count($event_box_data[$h_players[$intA]->id])) {
                foreach($event_box_data[$h_players[$intA]->id] as $box_event_id => $event_value) {
                    if (isset($event_box_fields[$box_event_id])) {
                        $insert_field .= ',boxfield_'.intval($event_box_fields[$box_event_id]);
                        $insert_vals .= ",".($event_value != ''? ($event_value): 'NULL');
                        if($update_vals){
                            $update_vals .= ",";
                        }
                        $update_vals .= "boxfield_".intval($event_box_fields[$box_event_id])."=".($event_value != ''? ($event_value):'NULL');
                    }
                }
            }
        ...
        for($intA=0;$intA<count($a_players);$intA++){
            ...
            if (isset($event_box_data[$a_players[$intA]->id]) && count($event_box_data[$a_players[$intA]->id])) {
                foreach($event_box_data[$a_players[$intA]->id] as $box_event_id => $event_value) {
                    if (isset($event_box_fields[$box_event_id])) {
                        $insert_field .= ',boxfield_'.intval($event_box_fields[$box_event_id]);
                        $insert_vals .= ",".($event_value != ''? ($event_value): 'NULL');
                        if($update_vals){
        ...
        if($score1_prev != $row->score1 || $score2_prev != $row->score2){
            classJsportPlugins::get('onMatchScoreChange', array('match_id' => $row->id));
        }
