...
$doc = JFactory::getDocument();
                $doc->addCustomTag('<link rel="stylesheet" type="text/css" href="'.jUri::base().'components/com_joomsport/sportleague/assets/css/drawBracketBE.css" />');
        ...
        $doc->addCustomTag('<link rel="stylesheet" type="text/css" href="//code.jquery.com/ui/1.12.1/themes/base/jquery-ui.css" />');
                ...
                $doc->addScript(JUri::base().'administrator/components/com_joomsport/js/knockcomplex.js');
                $doc->addScript(JUri::base().'components/com_joomsport/js/knockcomplex2.js');
class edit_matchdayJSModel extends JSPRO_Models
{
    ...
    public function getData()
    {
        ...
        if ($this->acl == 1) {
            ...
            } elseif ($this->t_type == 3 && $row->k_format) {
                
                //$this->_lists['format'] = $this->knock_type->getKnockFormat($row, $this->_lists['t_type']);
                $this->_lists['knock_layout'] = $this->knock_type->getKnockComplpex($row, $tourn, $this->_lists['match'], $this->season_id, $cfg, 1);
            }
    ...
    public function getFiltermday()
    {
        $query = 'SELECT m.* FROM #__bl_season_players as t,#__bl_players as p,#__bl_seasons as s,#__bl_matchday as m'
                .' WHERE s.published=1 AND m.s_id=s.s_id AND s.s_id=t.season_id AND t.player_id=p.id AND p.usr_id='.$this->_user->id.' AND s.s_id='.intval($this->season_id)
                .' ORDER BY m.id desc';
        if ($this->season_id == -1) {
        ...
        $query = 'SELECT COUNT(*) FROM #__bl_season_players as t,#__bl_players as p,#__bl_seasons as s,#__bl_matchday as m'
                .' WHERE s.published=1 AND m.s_id=s.s_id AND s.s_id=t.season_id AND t.player_id=p.id AND p.usr_id='.$this->_user->id.' AND s.s_id='.intval($this->season_id).' AND m.id='.$this->mid
                .' ORDER BY m.id desc';
        if ($this->season_id == -1) {
            $query = 'SELECT COUNT(*) FROM #__bl_matchday as m'
                .' WHERE m.s_id='.intval($this->season_id).' AND m.id='.$this->mid;
        }
        $this->db->setQuery($query);
        if (!$this->db->loadResult()) {
    ...
    public function getteamsSeas($sid)
    {
        if ($this->t_single || ($sid == -1 && $this->acl == 3)) {
            $query = "SELECT CONCAT(t.first_name,' ',t.last_name) as t_name,t.id"
                    ...
                    .' WHERE st.player_id = t.id AND st.season_id = '.intval($sid)
            ...
            $query = 'SELECT * FROM #__bl_teams as t , #__bl_season_teams as st'
                    .' WHERE st.team_id = t.id AND st.season_id = '.intval($sid)
                    .' ORDER BY t.t_name';
            if ($sid == -1) {
    ...
    public function getMdMatch()
    {
        $query = "SELECT m.*,md.m_name,md.id as mdid, CONCAT(t1.first_name,' ',t1.last_name) as home_team, CONCAT(t2.first_name,' ',t2.last_name) as away_team, t1.id as hm_id,t2.id as aw_id"
                ...
                .' WHERE m.m_id = md.id AND md.s_id='.intval($this->season_id).' AND (t1.usr_id = '.$this->_user->id.' OR t2.usr_id = '.$this->_user->id.')'
                .' AND md.id='.$this->mid
    ...
    public function AdmMDSave()
    {
        ...
        if($t_knock == '3'){
            require_once JPATH_SITE.'/components/com_joomsport/controllers/admin.php';
            $this->id = $row->id;
            
            $obb = new JoomsportControllerAdmin();
            return $obb->saveKnockComplex($row);
        }
        ...
        if ($this->acl == 1 && $t_knock) {
            if (isset($_POST['teams_kn']) && count($_POST['teams_kn'])) {
                foreach ($_POST['teams_kn'] as $home_team) {
                    ...
                    $match->m_id = $row->id;
                    $score1_prev = $match->score1;
                    $score2_prev = $match->score2;
...
//update	
                    ...
                    if($score1_prev != $match->score1 || $score2_prev != $match->score2){
                        classJsportPlugins::get('onMatchScoreChange', array('match_id' => $match->id));
                    }
                }
            }
