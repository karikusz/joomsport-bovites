...
<form action="index.php?option=com_joomsport" method="post" name="adminForm" id="adminForm">
		<?php
        if (!($lists['totplayer'])) {
            JhtmlJshtml::createmess('player_add');
        } else {
            ?>
		...
		<div id="filter-bar" class="btn-toolbar">
            ...
            <div class="btn-group pull-right"><?php echo JText::_('BLBE_FILTERS');
            ?>: <?php echo $this->lists['tourn'];
            ?><?php echo $this->lists['teams1'];
            ?></div>
		</div>
        ...
        if (count($rows)) {
            for ($i = 0, $n = count($rows); $i < $n; ++$i) {
                ...
                $link = JRoute::_('index.php?option=com_joomsport&task=player_edit&cid[]='.$row->id.'&seasf_id='.$this->season_id);
